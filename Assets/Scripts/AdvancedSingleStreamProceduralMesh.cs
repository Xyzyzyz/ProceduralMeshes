...
public class AdvancedSingleStreamProceduralMesh : MonoBehaviour {
	...
	void OnEnable () {
		...
		int vertexAttributeCount = 4; // Each vertex has 4 attributes -- position, normal, tangent, and texture coordinates
		int vertexCount = 4;
		...
		NativeArray<VertexAttributeDescriptor> vertexAttributes = new NativeArray<VertexAttributeDescriptor>(
			vertexAttributeCount, Allocator.Temp, NativeArrayOptions.UninitializedMemory // Allocate temporary native array for vertices, make it so that unity doesn't default the values to 0
		);
		...
		vertexAttributes[3] = new VertexAttributeDescriptor(
			VertexAttribute.TexCoord0, VertexAttributeFormat.Float16, dimension: 2
		);
